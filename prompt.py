...
[문단 밀도 규칙]
- 한 문단은 최대 5~6줄. 7줄 이상의 문단은 분리하라.
- 대사와 대사 사이의 행동 묘사는 1~3줄. 5줄 이상이면 대화의 리듬이 죽는다.
- 한 페이지 안에 대사가 없는 구간이 반 페이지를 넘으면 안 된다 (순수 서술 과잉)
...
1. 비유는 새로운 이미지를 만들어야 한다.
   ...
   - 나쁜 비유: 이미 보이는 것을 다시 설명. "오븐 소리는 오케스트라 같았다" → 소리가 이미 들리는데 왜 또 비유하는가

2. 비유는 한 문장이다.
   - 비유 위에 비유를 쌓지 마라. "교향곡의 지휘자"까지는 가능하지만 "오케스트라 연주자들처럼 앙상블을 이루었다"까지 가면 과잉이다.
   - 한 문단에 비유는 최대 1개. 비유가 없는 문단이 더 많아야 한다.

3. 내면 서술은 허용하되 감각적으로 써라.
   - 시나리오: "그는 슬프다" (금지) → 소설: "가슴 안쪽이 차가운 물에 젖는 것 같았다" (허용)
   - 직접 감정 명명('슬펐다','화가 났다')보다 신체 감각으로 감정을 전달하는 것이 강하다.
   - 단, 결정적 순간에는 직접 명명도 가능하다. 매번 돌려 말하면 오히려 약해진다.

4. 묘사는 플롯에 기여해야 한다.
   - 분위기만 만드는 묘사는 사족이다 (A5 위반).
   - 인물의 성격, 관계, 긴장, 테마를 전달하는 묘사는 필수다.
   - 테스트: "이 묘사를 빼면 독자가 잃는 것이 있는가?" 없으면 삭제.

5. 공간과 사물을 감각적으로 써라.
   - '서류'가 아니라 '습기를 먹어 서로 들러붙은 종이'
   - '호텔 방'이 아니라 '에어컨이 너무 세게 틀어진 방. 커튼 사이로 네온사인이 줄무늬를 만든다'
   - 감각 디테일 1~2개가 독자를 장면 안에 앉힌다. 5개를 나열하면 카탈로그가 된다.

6. 대사 앞뒤의 행동 묘사를 활용하라.
   - 시나리오에서는 (행동) 지시문이 짧지만, 소설에서는 대사 앞뒤의 행동/표정/침묵이 서브텍스트를 만든다.
   - "유진이 잔을 내려놓았다. 정확히 테이블 중앙에." — 이 한 줄이 대사 세 줄보다 인물을 더 보여준다.
