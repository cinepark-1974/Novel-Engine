...
AI 문체 교정 규칙 (반드시 준수):
A1. "~것이었다", "~터였다", "~셈이었다", "~따름이었다" 같은 해설체 종결을 쓰지 않는다. "변해 있을 터였다" → "변했다"처럼 단정적으로 끝낸다.
A2. 인물의 심리를 격언이나 철학적 진술로 풀어놓지 않는다. "높은 곳에서 내려다보면 얼마나 더 올라갈 수 있는지를 가늠하게 된다" 같은 문장은 쓰지 않는다. 심리는 행동, 선택, 한 줄 판단으로 보여준다.
A3. 모든 문장이 의미심장하려 하지 않는다. 정보만 전달하는 가벼운 문장, 빈 공간이 있어야 진짜 리듬이다.
A4. 장면 전환을 설명하지 않는다. "사람들이 하나둘 자리를 뜬 후 혼자 남았다" 대신 다음 행동으로 바로 넘어간다. "유진은 펜트하우스로 돌아와 그제서야 소파에 앉아 신발을 벗었다"처럼.
A5. 플롯에 기여하지 않는 분위기 묘사를 넣지 않는다. "서류를 가방에 넣었다", "케이터링 직원이 치우는 소리" 같은 사족을 경계한다.
A6. 인물 호칭은 자연스럽게 전환한다. 첫 등장은 풀네임, 이후 성 또는 이름으로 바꾼다. 끝까지 풀네임을 반복하지 않는다.
A7. "짧은문장. 짧은문장. 그리고 긴 문장" 패턴을 기계적으로 반복하지 않는다. 리듬 변화는 있되 같은 패턴이 두 번 이상 연속하면 깨뜨린다.
A8. 실존 호텔, 기업, 장소 이름을 그대로 쓰지 않는다. 실제 느낌이 나는 가상의 이름을 만든다.
A9. 단락은 흐름이 있는 문단으로 묶는다. 매 문장마다 줄바꿈하지 않는다.
""".strip()
...
문체 금지 패턴:
- "~것이었다", "~터였다", "~셈이었다", "~따름이었다" 종결 금지. 단정형("~했다", "~였다")으로 끝낸다.
- 인물 심리를 격언, 비유, 철학적 진술로 풀지 않는다. 판단은 한 문장, 행동은 바로 다음 문장.
- 장면 전환 시 "~한 후", "~하고 나서" 같은 설명적 연결 대신 다음 행동으로 바로 넘어간다.
- 플롯에 기여하지 않는 분위기 묘사(배경음, 조명, 소품 정리 등)를 넣지 않는다.
- 인물 풀네임을 끝까지 반복하지 않는다. 첫 등장 후 성 또는 이름으로 전환한다.
- "짧은문장. 짧은문장. 그리고 긴 문장" 패턴을 기계적으로 반복하지 않는다.
- 실존 호텔, 기업, 장소 이름을 그대로 쓰지 않는다.
- 매 문장마다 줄바꿈하지 않는다. 흐름이 있는 문단으로 묶는다.
""".strip()
...
챕터 제목 규칙:
- 원고의 첫 줄에 반드시 챕터 제목을 출력할 것
- 형식: [CHAPTER {unit_no}] — 서브타이틀
- 서브타이틀은 이 챕터에서 독자가 가장 기억할 것을 기준으로 선택한다
- 주 배경 도시/장소가 핵심이면 장소명을 쓴다 (예: 뉴욕 플래티넘 호텔)
- 특정 인물의 등장이나 대결이 핵심이면 인물명을 쓴다 (예: 마리아 황)
- 매 챕터 같은 유형만 반복하지 말고, 작품 전체에서 장소와 인물이 자연스럽게 섞이게 한다
- 챕터 제목 다음 줄부터 바로 원고 본문을 시작할 것
...
- 원고의 첫 줄에 반드시 챕터 제목을 출력할 것
- 형식: [CHAPTER 13] — 에필로그
- 서브타이틀은 "에필로그"를 기본으로 하되, 장소나 인물이 더 적합하면 바꿔도 좋다
- 챕터 제목 다음 줄부터 바로 에필로그 본문을 시작할 것
